...
async function register(payload) {
    ...
    if (pass.length < 6) throw new ApiError(400, "Password must be at least 6 chars");
    if (pass.length > 50) throw new ApiError(400, "Password must be <= 50 chars");

    const hash = await bcrypt.hash(pass, 10);

    const [id] = await knex("USER").insert({
        ROLE_ID: payload.ROLE_ID,
        U_Password: hash, // LƯU HASH
        U_Fullname: payload.U_Fullname,
        U_Email: payload.U_Email,
        U_Phone: payload.U_Phone || null,
        U_Address: payload.U_Address || null,
        U_Gender: payload.U_Gender || null,
        U_Birthday: payload.U_Birthday || null,
...
async function listUsers(query = {}) {
    ...
    const filters = (qb) => {
        ...
        if (query.q) {
            qb.where((b) =>
                b
                    .orWhere("U.U_Fullname", "like", `%${query.q}%`)
...
async function updateProfile(req, body) {
    ...
    await knex("USER")
        ...
        .where("U_ID", req.session.user_id);
...
async function changePassword(req, body) {
    ...
    if (String(new_password).length < 6) throw new ApiError(400, "Password must be at least 6 chars");
    if (String(new_password).length > 50) throw new ApiError(400, "Password must be <= 50 chars");

    const row = await knex("USER").select("U_Password").where("U_ID", req.session.user_id).first();
    if (!row) throw new ApiError(404, "User not found");

    const ok = await bcrypt.compare(String(current_password), String(row.U_Password || ""));
    if (!ok) throw new ApiError(400, "Current password incorrect");

    const nextHash = await bcrypt.hash(String(new_password), 10);
    await knex("USER")
        .update({ U_Password: nextHash, Updated_at: knex.fn.now() })
        .where("U_ID", req.session.user_id);
...
async function updateUserStatus(req, id, status) {
    ...
    const ret = await knex("USER")
        .update({ U_Status: status, Updated_at: knex.fn.now() })
        .where("U_ID", id);
...
async function updateUser(userId, updates) {
    // Nếu có đổi password qua admin, vẫn hash
    if (typeof updates.U_Password === "string" && updates.U_Password) {
        if (updates.U_Password.length < 6) throw new ApiError(400, "Password must be at least 6 chars");
        if (updates.U_Password.length > 50) throw new ApiError(400, "Password must be <= 50 chars");
        updates.U_Password = await bcrypt.hash(String(updates.U_Password), 10);
    }
